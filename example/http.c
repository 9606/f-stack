...
static int http_tcpclient_create(const char *host, int port) {
    ...
    if ((socket_fd = ff_socket(AF_INET, SOCK_STREAM, 0)) == -1) {
        printf("ff_socket failed\n");
        return -1;
    }

    int on = 1;
    ff_ioctl(socket_fd, FIONBIO, &on);

    ff_connect(socket_fd, (struct linux_sockaddr *) &server_addr, sizeof(server_addr));
...
static int http_tcpclient_recv(int socket, char *lpbuff) {
    ...
    recvnum = ff_recv(socket, lpbuff, BUFFER_SIZE * 4, 0);
...
static int http_tcpclient_send(int socket, char *buff, int size) {
    ...
    while (sent < size) {
        tmpres = ff_send(socket, buff + sent, size - sent, 0);
        if (tmpres == -1) {
            return -1;
        }
        sent += tmpres;
...
int main(int argc, char *argv[]) {
    ...
    memcpy(url, argv[argc - 1], strlen(argv[argc - 1]) + 1);
